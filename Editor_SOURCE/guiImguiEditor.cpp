...
namespace gui
{
	...
	void ImguiEditor::Initialize()
	{
		...
		tg::graphics::GraphicDevice_DX11*& graphicdevice = tg::graphics::GetDevice();
	...
	void ImguiEditor::OnEvent(tg::Event& e)
	{
		// Block event when ImGui is capturing mouse or keyboard
		if (mbBlockEvent)
		{
			ImGuiIO& io = ImGui::GetIO();
			e.Handled |= e.IsInCategory(tg::eEventCategory::EventCategoryMouse) & io.WantCaptureMouse;
			e.Handled |= e.IsInCategory(tg::eEventCategory::EventCategoryKeyboard) & io.WantCaptureKeyboard;
		}
	}
